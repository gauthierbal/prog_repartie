...
public class DessinHorloge{
  ...
  public static void dessinerNumeros(Graphics gTmp, int pas, int rayon, Color couleur){
	  ...
	  for (int i=1; i<13; i++){
		  if(i%pas == 0){
			  ...
			  String chf = (new Integer(i)).toString();
  ...
  /* dessin de l'horloge */
  public static void dessineHorloge(Graphics gTmp, int haut, int larg, int heure, double minutes, double secondes){


	int taille = (haut < larg) ? haut : larg;

    Color cFond = new Color(37, 58, 91);
    ...
    DessinHorloge.dessinerNumeros(gTmp,1,taille,new Color(252, 255, 79));

    int centreX = taille/2;
    int centreY = taille/2;

    //Aiguille des secondes
    DessinHorloge.dessinerAiguille(gTmp,secondes/60.0, centreX, centreY, 0.45*taille,0.01*taille,new Color(165, 34, 34));

    //Aiguille des minutes
    DessinHorloge.dessinerAiguille(gTmp,minutes/60.0, centreX, centreY, 0.50*taille,0.02*taille,new Color(65, 183, 29));


    //Aiguille des heures
    DessinHorloge.dessinerAiguille(gTmp,heure/12.0+minutes/60.0/12.0, centreX, centreY, 0.25*taille,0.03*taille,new Color(155, 155, 155));
